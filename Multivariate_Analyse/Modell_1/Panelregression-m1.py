...
plt.title("Negative-Binomial-Modelle: Protestaktivität (t-1) → Instagram")
...
# =========================
# 7. Tabellen für jede Aggregation + Überdispersion
# =========================
...
# NB-Ergebnisse: Plot + Terminal-Ausgabe
for agg in ["Daily","Weekly","Monthly"]:
    df_agg = results_nb[results_nb["model"]==agg]
    print_nb_results_table(df_agg, f"{agg} Aggregation: Proteste → Instagram")
    plot_colored_results_table(df_agg, f"{agg} Aggregation: Proteste → Instagram")

# =========================
# Überdispersion der Zählvariablen
# =========================
dispersion_data = []
aggregations = [("Daily", df), ("Weekly", df_week), ("Monthly", df_month)]
dvs = ["Posts", "Active_Accounts"]
...
print("\n===== Überprüfung der Poisson-Annahme für die Zählvariablen =====")
print(disp_df.to_string(index=False))
