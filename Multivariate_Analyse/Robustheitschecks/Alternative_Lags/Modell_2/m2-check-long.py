...
# =========================
# 2. Lagged Variablen (Lag 3)
# =========================
lag_vars = ["Protests","participants","Posts","Active_Accounts"]
for var in lag_vars:
    df[f"{var}_lag3"] = df.groupby("Location")[var].shift(3).fillna(0)
...
for var in lag_vars:
    df_week[f"{var}_lag3"] = df_week.groupby("Location")[var].shift(3).fillna(0)
...
for var in lag_vars:
    df_month[f"{var}_lag3"] = df_month.groupby("Location")[var].shift(3).fillna(0)
...
# =========================
# 5. Modellfunktion (M2 Lag 3)
# =========================
def run_model_insta_to_protest_nb(data, time_var, dv, label):
    formula = f"{dv} ~ Posts_lag3 + Active_Accounts_lag3 + C(Location) + C({time_var})"
    ...
    results = pd.DataFrame({
        "variable": ["Posts_lag3","Active_Accounts_lag3"],
        "coef": model.params[["Posts_lag3","Active_Accounts_lag3"]],
        "std_err": model.bse[["Posts_lag3","Active_Accounts_lag3"]],
        "dv": dv,
        "model": label
    })
...
results_nb["Variable"] = results_nb["variable"].replace({
    "Posts_lag3": "Posts",
    "Active_Accounts_lag3": "Active Accounts"
})
...
# 9. Excel Export
# =========================
output_file = "Model_Results_M2_Lag3.xlsx"

with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    final_table.to_excel(writer, sheet_name="NB_Model_Results_M2_Lag3", index=False)
