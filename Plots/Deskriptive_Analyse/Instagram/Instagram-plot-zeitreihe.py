...
# =============================================
# 11. Plot 4: Aktive Accounts
# =============================================
# 7-Tage-Durchschnitt berechnen
df['active_accounts_7d'] = df['Active_Accounts'].rolling(window=7).mean()

plt.figure(figsize=(12,6))
plt.plot(df['Date'], df['Active_Accounts'], color='lightgray', label='Tägliche aktive Accounts')
plt.plot(df['Date'], df['active_accounts_7d'], color='orange', linewidth=2, label='7-Tage-Durchschnitt')
plt.xlabel('Zeit')
plt.ylabel('Anzahl aktiver Accounts')
plt.title('Zeitliche Entwicklung der aktiven Accounts')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
# -----------------------------
# 12. Plot 5: Tägliche Posts
# -----------------------------

# 7-Tage-Durchschnitt berechnen
df['posts_7d'] = df['Posts'].rolling(window=7).mean()

# Plot erstellen
plt.figure(figsize=(12,6))
plt.plot(df['Date'], df['Posts'], color='lightgray', label='Tägliche Posts')
plt.plot(df['Date'], df['posts_7d'], color='blue', linewidth=2, label='7-Tage-Durchschnitt')
plt.xlabel('Zeit')
plt.ylabel('Anzahl Posts')
plt.title('Zeitliche Entwicklung der Posts')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
