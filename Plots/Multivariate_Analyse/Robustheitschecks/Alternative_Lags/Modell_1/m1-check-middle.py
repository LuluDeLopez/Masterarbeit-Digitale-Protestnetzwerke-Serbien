...
# =========================
# 2. Lagged Variablen erstellen (Lag 2)
# =========================
...
# =========================
# 5. Modellfunktion (Lag 2)
# =========================
def run_model_protest_to_insta_nb(data, time_var, dv, label):
    formula = f"{dv} ~ Protests_lag2 + participants_lag2 + C(Location) + C({time_var})"
    ...
    results = pd.DataFrame({
        "variable": ["Protests_lag2","participants_lag2"],
        "coef": model.params[["Protests_lag2","participants_lag2"]],
        "std_err": model.bse[["Protests_lag2","participants_lag2"]],
        "dv": dv,
        "model": label
    })
...
# 7. Export nach Excel
# =========================
output_file = "Model_Results_Lag2.xlsx"

with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    results_nb.to_excel(writer, sheet_name="NB_Model_Results_Lag2", index=False)
